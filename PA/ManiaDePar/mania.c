...
listadj_noh_pont createNoh(int vert, int cst)
{
...
void addAresta(graphL_pont graph, int source, int destination, int cst){
    // cria o nó, seta o cabeçario da lista para o nó criado
    listadj_noh_pont novoNoh = createNoh(destination,cst);
...
{
    int cities = 0, roads = 0; //cidades identificadas por números interios de um a C
    int source = 0, dest = 0, toll = 0;
    int nodes = 0;
    int rCost [10000];
    int i = 0;
    return 0;
    scanf("%d%d",&cities,&roads);
    graphL_pont roadmap =  createGraphL(cities);
    for (i = 0; i < roads; i++){
        scanf("%d%d%d",&source,&source,&toll);
        addAresta(roadmap,source,dest,toll);
    }
}
